...
<body>
    <header>
        <h1> Witaj 
        <?php 
            $user_Id = Id_z_url_user(); 
            $parent_Id = Id_z_url_parent();
            $imie_ucznia= Imie_ucznia_z_userID($user_Id); 
            $imie_rodzica= Imie_rodzica_z_parent_Id($parent_Id);
            echo($imie_rodzica." na panelu ".$imie_ucznia); 
        ?>
        !
        </h1>
    </header>
    ...
    <?php 
        ...
        function Imie_rodzica_z_parent_Id($parent_Id) {
            // Połączenie z bazą danych
            $DBHOST = "localhost";
            $DBUSER = "admin";
            $DBPASS = "admin";
            ...
            $conn = mysqli_connect($DBHOST, $DBUSER, $DBPASS, $DBNAME);      
            // Zapytanie SQL
            $sql = "SELECT Imie  FROM rodzice WHERE id_rodzica = $parent_Id";
